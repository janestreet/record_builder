...
(** A subset of the operations of an {{!modtype:Base.Applicative.S2} applicative} with two
    type parameters. *)
...
(** The types used internally for building records by folding over fields — you shouldn't
    need to know them, but they must be exposed to allow type-checking to succeed. *)
module type Make_creator_types = sig
  ...
  (** A step of the fold over one field.
      ...
      Each argument to [Fields.make_creator] should take that field as an argument and
      return something of this type. *)
  ...
  (** A step of the fold over all fields of a record.
      ...
      The application of [Fields.make_creator] to all of the fields, but without an
      initial value for the fold, should have this type and be passed to
      {!Record_builder.S.build_for_record}. *)
...
(** Modules of this type are used to traverse a record using a specific applicative. They
    can be made using the functor {!Record_builder.Make}.
    ...
    {[
      let form : t G.t =
        ...
        in
        G.both
          (labelled_string Field.forename)
...
module type Record_builder_S = sig
  ...
  (** The types used internally for building records by folding over fields — you
      shouldn't need to know them, but they must be exposed to allow type-checking to
      succeed. *)
  ...
  (** Build the overarching applicative for the whole record.
      ...
      should supply no initial value but use {!field} to supply a term for every field of
      the record.
